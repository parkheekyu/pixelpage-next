...
export interface MetaInsight {
  ...
  link_clicks: number; landing_page_views: number; leads: number; purchases: number; cpl: number | null;
...
function toInsight(r: RawInsight | undefined): MetaInsight | null {
  ...
  return {
    ...
    link_clicks: n(r.inline_link_clicks), landing_page_views: act(r.actions, "landing_page_view"), leads, purchases: act(r.actions, "purchase", "offsite_conversion.fb_pixel_purchase"),
...
function sum(list: (MetaInsight | null)[]): MetaInsight {
  const t: MetaInsight = { spend: 0, impressions: 0, reach: 0, frequency: 0, clicks: 0, ctr: 0, cpc: 0, cpm: 0, link_clicks: 0, landing_page_views: 0, leads: 0, purchases: 0, cpl: null, video_p25: 0, video_p50: 0, video_p75: 0, video_p100: 0, thruplays: 0, hook_rate: null, hold_rate: null };
  for (const i of list) if (i) for (const k of ["spend", "impressions", "reach", "clicks", "link_clicks", "landing_page_views", "leads", "purchases", "video_p25", "video_p50", "video_p75", "video_p100", "thruplays"] as const) t[k] += i[k];
  ...
  t.frequency = t.reach ? t.impressions / t.reach : 0; t.cpl = t.leads ? t.spend / t.leads : null; t.hook_rate = t.impressions && t.thruplays ? t.thruplays / t.impressions : null; t.hold_rate = t.video_p25 ? t.video_p100 / t.video_p25 : null;
  return t;
}
...
export async function fetchMetaSnapshot(adAccountId: string, datePreset = "last_30d"): Promise<MetaAccountSnapshot> {
  ...
  const withData = adList.filter((a) => a.insight && a.insight.spend > 0);
  const winners = [...withData].sort((a, b) => {
  ...
  return { account, date_preset: datePreset, fetched_at: new Date().toISOString(), campaigns: campList, totals: sum(adList.map((a) => a.insight)), winners };
}

/** AI 프롬프트용 압축 텍스트 */
export function snapshotToText(s: MetaAccountSnapshot): string {
  const won = (v: number) => "₩" + Math.round(v).toLocaleString("ko-KR");
  const pct = (v: number | null) => (v == null ? "-" : (v * 100).toFixed(1) + "%");
  const line = (i: MetaInsight | null) => i ? `spend ${won(i.spend)} · imp ${i.impressions} · ctr ${i.ctr.toFixed(2)}% · cpc ${won(i.cpc)} · cpm ${won(i.cpm)} · freq ${i.frequency.toFixed(2)} · link ${i.link_clicks} · lpv ${i.landing_page_views} · leads ${i.leads} · cpl ${i.cpl == null ? "-" : won(i.cpl)}${i.thruplays ? ` · hook ${pct(i.hook_rate)} · hold ${pct(i.hold_rate)}` : ""}` : "(성과 없음)";
  const out: string[] = [`계정 ${s.account.name ?? s.account.id} (${s.account.currency ?? ""}) · 기간 ${s.date_preset}`, `합계: ${line(s.totals)}`, ""];
