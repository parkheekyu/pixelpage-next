...
import type { MetaAccountSnapshot } from "@/lib/integrations/meta";
...
export default async function AdsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ preset?: string; refresh?: string }> }) {
  ...
  if (isStaff && integ?.meta_ad_account_id) {
    try { const r = await getMetaSnapshot(supabase, project.id, integ.meta_ad_account_id, preset, sp.refresh === "1"); snapshot = r.snapshot; cachedAt = r.cached_at; } catch (e) { snapError = (e as Error).message; }
  }
  return <AdsClient project={project} isStaff={isStaff} adAccountId={integ?.meta_ad_account_id ?? null} preset={preset} snapshot={snapshot} snapError={snapError} cachedAt={cachedAt} analyses={analyses} tokenConfigured={!!process.env.META_ACCESS_TOKEN} />;
}
