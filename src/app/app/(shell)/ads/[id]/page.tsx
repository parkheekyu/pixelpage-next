...
export default async function AdsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ preset?: string; refresh?: string }> }) {
  ...
  return <><ProjectNav project={project} isStaff={profile.role === "staff"} unread={unread} />
    <AdsClient project={project} isStaff={isStaff} adAccountId={integ?.meta_ad_account_id ?? null} preset={preset} snapshot={snapshot} snapError={snapError} cachedAt={cachedAt} analyses={analyses} tokenConfigured={!!process.env.META_ACCESS_TOKEN} /></>;
}
